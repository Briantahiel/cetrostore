    ...
    <article className="flex h-full min-h-[430px] flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:border-blue-300 hover:shadow-xl hover:shadow-blue-950/10 dark:border-slate-700 dark:bg-slate-950 dark:hover:border-cyan-400">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <Link href={detailHref} onClick={onOpen} className="mt-3 block">
          <h3 className="text-xl font-black tracking-tight text-slate-950 transition hover:text-blue-700 dark:text-white dark:hover:text-cyan-300">
            {nombre}
          </h3>
        </Link>

        <p className="mt-3 flex-1 text-sm font-semibold leading-6 text-slate-700 dark:text-slate-300">
          {descripcion}
        </p>
        ...
        {color || variantes?.length ? (
          <div className="mt-4 flex flex-wrap gap-2">
            {color ? (
              <span className="rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-xs font-black text-blue-800 dark:border-cyan-300/30 dark:bg-cyan-300/10 dark:text-cyan-200">
                {color}
              </span>
            ) : null}
            {variantes?.map((variante) => (
              <span
                key={variante.codigo}
                className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-black text-slate-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300"
              >
                {variante.color}
              </span>
            ))}
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          <Link
            href={detailHref}
            onClick={onOpen}
            className="flex items-center justify-center rounded-lg border border-slate-300 px-4 py-3 text-center text-sm font-black text-slate-800 transition hover:border-blue-400 hover:bg-blue-50 hover:text-blue-700 dark:border-slate-600 dark:text-slate-100 dark:hover:border-cyan-400 dark:hover:bg-slate-900 dark:hover:text-cyan-300"
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center justify-center rounded-lg bg-blue-700 px-4 py-3 text-center text-sm font-black text-white transition hover:bg-slate-950 dark:bg-cyan-300 dark:text-slate-950 dark:hover:bg-white"
            style={{ minHeight: "3rem" }}
          >
            Consultar
          </a>
